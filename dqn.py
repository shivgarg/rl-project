...
from neural_agent_dqn import NeuralAgentDQN
...
def play(agent, path, max_step=100, nb_episodes=10, verbose=True):
    ...
    for no_episode in range(nb_episodes):
        ...
        max_score = 0
        while not done: 
            command, command_id, Q = agent.act(obs, score, done, infos)
            episode.append({'obs':obs, 'infos':infos, 'command_id':command_id, 'Q': Q})
            last_score = score
        ...
        if (no_episode+1)%UPDATE_FREQ == 0 and agent.mode!='test':
